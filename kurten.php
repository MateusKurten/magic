...
&nbsp;&nbsp;&nbsp;&nbsp;
<INPUT TYPE="hidden" NAME="idAmigo" VALUE=1>
...
<?php
	$arrayCarta = DBReadQuery("select c.Nome from Carta c, AmigoCarta ac where ac.idAmigo = 1 and ac.idCarta = c.idCarta;");
...
<?php
	$arrayAmigo = DBRead("Nome", "Amigo", "where idAmigo != 1");
...
<select id="nomeCartaEmprestimo" name="nomeCarta">
<?php
	$arrayCarta = DBReadQuery("select c.Nome from Carta c, Emprestimo e where e.idDono = 1 and e.idCarta = c.idCarta;");
	for ($i = 0; $i < count($arrayCarta); $i++){
		echo ("<option>".$arrayCarta[$i]["Nome"]."</option>");
	}
?>
...
<select id="amigoEmprestimoCadastro" name="nomeAmigo">
<?php
	$arrayAmigo = DBReadQuery("select distinct a.Nome from Amigo a, Emprestimo e where e.idDono = 1 and e.idDevedor = a.idAmigo;");
	for ($i = 0; $i < count($arrayAmigo); $i++){
		echo ("<option>".$arrayAmigo[$i]["Nome"]."</option>");
	}
?>
